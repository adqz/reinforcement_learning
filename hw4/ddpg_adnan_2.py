...
class DDPG():
    ...
    def train(self, max_num_steps):
        ...
        for num_steps in tqdm(range(max_num_steps)):
            ...
            a_target = self.actor_target(s_prime).detach()                    #dim(batch_size, action_dim)
            y_i = r + gamma*self.critic_target(s_prime, a_target).detach()    #dim (batch_size, 1)
...
if __name__ == "__main__":
    ...
    ddpg_object = DDPG(
        ...
        gamma=0.99,
        batch_size=250,
        ev_n_steps=250, #evaluate every n steps
        verbose=verbose
    )
    # Train the policy
    ddpg_object.train(int(5e4))
